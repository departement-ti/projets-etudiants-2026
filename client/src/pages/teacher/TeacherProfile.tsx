// client/src/pages/teacher/TeacherProfile.tsx
...
export default function TeacherProfile() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      // 1. تحديث البيانات في قاعدة البيانات (Backend)
      await axios.put(`${API}/profile`, formData, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });

      // 2. ✅ تحديث التخزين المحلي (localStorage) فوراً لتغيير الاسم في الواجهة
      ...
      fetchProfile(); // إعادة جلب البيانات لتحديث واجهة البروفايل
      
      // اختياري: عمل تحديث بسيط للصفحة إذا كانت المكونات الأخرى لا تستجيب فوراً
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === "info" && (
        <div className="rounded-2xl bg-white p-6 shadow-sm">
          <h2 className="mb-6 text-lg font-semibold text-slate-800">Modifier les informations</h2>
      ...
      {activeTab === "password" && (
        <div className="rounded-2xl bg-white p-6 shadow-sm">
          <h2 className="mb-6 text-lg font-semibold text-slate-800">Changer le mot de passe</h2>
